...
import { playerOfWeekMarkup, calculatePlayerOfWeek } from "./player-of-week.js?v=12.3";
...
function showLoading() {
  const markup = playerOfWeekMarkup(null, { compact: true });
...
function render(data) {
  ...
  const markup = playerOfWeekMarkup(award, { compact: true });
  if (desktopHost) desktopHost.innerHTML = markup;
  if (mobileHost) mobileHost.innerHTML = markup;
}
...
function showError(message) {
  ...
  for (const host of [desktopHost, mobileHost]) {
    if (!host) continue;
    host.innerHTML = '<section class="player-week-card is-empty is-compact"><div class="pow-loading"><i class="fa-solid fa-triangle-exclamation"></i><span></span></div></section>';
